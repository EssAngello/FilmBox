...
public class ImageAdapter {
    ...
    public static final String IMAGEPROVIDER_URL = "https://image.tmdb.org/t/p/w500";
    ...
    public static void setProviderImageURL(ImageView imageView, String path){
        try {
            imageView.setAlpha(0f);
            Picasso.get().load(IMAGEPROVIDER_URL + path).noFade().into(imageView, new Callback() {
                @Override
                public void onSuccess() {
                    imageView.animate().setDuration(500).alpha(1f).start();
                }
